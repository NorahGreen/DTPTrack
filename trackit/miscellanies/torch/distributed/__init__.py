...
def init_torch_distributed(device: str, use_aux_process_group: bool = True):
    ...
    nccl_timeout = timedelta(seconds=60)
    gloo_timeout = timedelta(seconds=60)
    ...
    if dist_backend != 'nccl':
        kwargs['timeout'] = gloo_timeout
    else:
        kwargs['timeout'] = nccl_timeout
    ...
    if dist_backend == 'nccl' and use_aux_process_group:
        ...
        _group_aux = dist.new_group(backend=aux_dist_backend, timeout=gloo_timeout)
...
def cleanup_torch_distributed():
    ...
    if _dist_enabled:
        ...
        print('| torch.distributed disabled\n', flush=True, end='')
        _dist_enabled = False
