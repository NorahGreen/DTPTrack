...
def apply_lora(model: nn.Module, lora_module_names: List[str],
               ...
               lora_dropout: float = 0.0, use_rslora: bool = False, init_method: str = 'bert'):
        ...
        *parent_path, child_module_name = lora_module_name.split(".")
        ...
        setattr(parent_module, child_module_name,
                wrapper_cls(getattr(parent_module, child_module_name), lora_r,
                            lora_alpha,
                            lora_dropout, use_rslora, init_method))


def enable_lora_(self: nn.Module, lora_r: int, lora_alpha: float, lora_dropout: float, use_rslora: bool, init_method: str):
    """
    ...
    for i_layer, block in enumerate(self.blocks):
        ...
        apply_lora(block, linear_names, lora_r, lora_alpha, lora_dropout, use_rslora, init_method)
    ...
    self.state_dict = state_dict.__get__(self, self.__class__)
    self.load_state_dict = load_state_dict.__get__(self, self.__class__)
        ...
        for idx, tag in enumerate(("q", "k", "v")):
            base_w = _pop_first(state_dict,
                                [f"{prefix}{tag}.linear.weight",
                                 f"{prefix}{tag}.weight"])
            if base_w is None:
                base_w = module.weight.data[idx * dim:(idx + 1) * dim]
