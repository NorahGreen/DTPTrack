...
def _build_backbone(backbone_config: dict, load_pretrained=True, torch_jit_trace_compatible=True,
    ...
    elif backbone_config['type'] == 'MAE':
        from .mae.mae import build_mae_backbone
        with torch.device(device), set_default_dtype(dtype):
            backbone = build_mae_backbone(load_pretrained=load_pretrained, **backbone_build_params)
    elif backbone_config['type'] == 'ViTDet':
        from .mae.vitdet import build_vitdet_backbone
        with torch.device(device), set_default_dtype(dtype):
            backbone = build_vitdet_backbone(load_pretrained=load_pretrained, **backbone_build_params)
    elif backbone_config['type'] == 'CMT':
        from .cmt.cmt import build_cmt_backbone
        with torch.device(device), set_default_dtype(dtype):
            backbone = build_cmt_backbone(load_pretrained=load_pretrained, **backbone_build_params)
    ...
    elif backbone_config['type'] == 'DINOv3':
        from .dinov3_custom.builder import build_dino_v3_backbone
        with torch.device(device), set_default_dtype(dtype):
            backbone = build_dino_v3_backbone(load_pretrained=load_pretrained, **backbone_build_params)
    elif backbone_config['type'] == 'webssl':
        from .webssl.builder import build_webssl_backbone
        with torch.device(device), set_default_dtype(dtype):
            backbone = build_webssl_backbone(load_pretrained=load_pretrained, **backbone_build_params)
    elif backbone_config['type'] == 'radio':
        from .radio.builder import build_radio_model
        with torch.device(device), set_default_dtype(dtype):
            backbone = build_radio_model(**backbone_build_params)
    elif backbone_config['type'] == 'iBOT':
        from .ibot.builder import build_ibot_backbone
        with torch.device(device), set_default_dtype(dtype):
            backbone = build_ibot_backbone(load_pretrained=load_pretrained, **backbone_build_params)
    elif backbone_config['type'] == 'VideoMAE_v2':
        from .video.videomae_v2.builder import build_videomae_v2
        with torch.device(device), set_default_dtype(dtype):
            backbone = build_videomae_v2(backbone_build_params['name'], load_pretrained)
    elif backbone_config['type'] == 'mae_lite':
        from .mae_lite.mae_lite import build_mae_lite_backbone
        name = backbone_build_params['name']
        pretrained = backbone_config['weight_path']
        with torch.device(device), set_default_dtype(dtype):
            backbone = build_mae_lite_backbone(model_name=name, pretrained=pretrained, **backbone_build_params)
    elif backbone_config['type'] == 'VSSM':
        from .VMamba.classification.models import build_model
        with torch.device(device), set_default_dtype(dtype):
            backbone = build_model(backbone_config)
    elif backbone_config['type'] == 'VIM':
        import trackit.models.backbone.vim.modeling as modeling
        func = getattr(modeling, backbone_config['model_name'])
        with torch.device(device), set_default_dtype(dtype):
            backbone = func(pretrained=load_pretrained, **backbone_build_params)
    else:
