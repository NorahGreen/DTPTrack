...
class BoxWithScoreMapLabelGenerator:
    def __init__(self, response_map_size,
                 template_size,
                 search_region_size):
        self.response_map_size = np.array(response_map_size)
        self.search_region_size = np.array(search_region_size)
        self.stride = self.search_region_size / self.response_map_size
        self.template_size = np.array(template_size)
        self.template_response_map_size = (self.template_size / self.stride).astype(np.int64)
    ...
    def __call__(self, training_pair, context: dict, data: dict, _, length_z: int, length_x: int):
        ...
        if context['is_positive']:
            for i in range(1, length_z):
                x_cropped_bbox = context[f'z_{i}_cropped_bbox']
                positive_sample_indices = positive_sample_assignment(x_cropped_bbox, self.template_response_map_size,
                                                                 self.template_size)
                assert x_cropped_bbox.ndim == 1
                assert x_cropped_bbox.size == 4
                x_cropped_bbox = x_cropped_bbox.astype(np.float64)
                normalized_bbox = x_cropped_bbox.copy()
                normalized_bbox[::2] = x_cropped_bbox[::2] / self.template_size[0]
                normalized_bbox[1::2] = x_cropped_bbox[1::2] / self.template_size[1]
                normalized_bbox = normalized_bbox.astype(np.float32)

                data[f'label_{index}'] = positive_sample_indices, normalized_bbox
                index += 1

            for i in range(length_x):
                x_cropped_bbox: np.ndarray = context[f'x_{i}_cropped_bbox']
...
def box_with_score_map_label_collator(batch: Sequence[Mapping], collated: TrainData, max_z: int, max_x: int):
    index = 0
    while True:
        if f'label_{index}' not in batch[0]:
            break

        label_list = tuple(data[f'label_{index}'] for data in batch)
        ...
        if collated_batch_ids is not None:
            assert collated_batch_ids.shape == collated_positive_sample_indices.shape

        collated.target.update({f'num_positive_samples_{index}': num_positive_samples,
                                f'positive_sample_batch_dim_indices_{index}': collated_batch_ids,
                                f'positive_sample_map_dim_indices_{index}': collated_positive_sample_indices,
                                f'boxes_{index}': collated_gt_bboxes})
        index += 1
