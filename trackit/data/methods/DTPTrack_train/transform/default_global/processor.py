...
from trackit.core.utils.tile_crop import resize_with_pad, _get_max_res_without_distortion
...
class SPMTrackDataProcessor(SPMTrackTrain_DataTransform):
    def __init__(self,
                 template_siamfc_cropping_parameters: SiamFCCroppingParameter | Sequence[SiamFCCroppingParameter],
                 search_image_size: Tuple[int, int],
                 augmentation_pipeline: AugmentationPipeline,
                 norm_stats_dataset_name: str,
                 additional_processors: Optional[Sequence[ExtraTransform]] = None):
        self.template_siamfc_cropping = SiamFCCropping(template_siamfc_cropping_parameters) if isinstance(template_siamfc_cropping_parameters, SiamFCCroppingParameter) else \
            tuple(SiamFCCropping(param) for param in template_siamfc_cropping_parameters)
    ...
    def __call__(self, training_pair: SiameseTrainingMultiPair, rng_engine: np.random.Generator):
        ...
        if isinstance(self.template_siamfc_cropping, SiamFCCropping):
            template_siamfc_cropping = [self.template_siamfc_cropping for _ in range(len(training_pair.template))]
        else:
            template_siamfc_cropping = self.template_siamfc_cropping

        for i in range(len(training_pair.template)):
            if not template_siamfc_cropping[i].prepare(f'z_{i}', rng_engine, context):
                return None
        ...
        for i in range(len(training_pair.template)):
            template_siamfc_cropping[i].do(f'z_{i}', context)
        for i in range(len(training_pair.search)):
            # search_region_siamfc_cropping[i].do(f'x_{i}', context)
            _resize_image_with_pad(f'x_{i}', context,
                                   self.search_image_size,
                                   self.template_siamfc_cropping.__getitem__(i).siamfc_cropping_parameter.interpolation_mode)
        ...
        for i in range(len(training_pair.search)):
            _bbox_clip_to_image_boundary_(context[f'x_{i}_resized_bbox'], context[f'x_{i}_resized_image'])

            self.image_normalize_transform_(context[f'x_{i}_resized_image'])
        ...
        for i in range(len(training_pair.search)):
            data[f'x_{i}_resized_image'] = context[f'x_{i}_resized_image']
    ...
    def _do_augmentation(self, context: dict, rng_engine: np.random.Generator, template_count: int, search_count: int):
        ...
        for i in range(search_count):
            augmentation_context[f'search_region_{i}'] = [AnnotatedImage(context[f'x_{i}_resized_image'], context[f'x_{i}_resized_bbox'])]
        ...
        for i in range(search_count):
            context[f'x_{i}_resized_image'] = augmentation_context[f'search_region_{i}'][0].image
            context[f'x_{i}_resized_bbox'] = augmentation_context[f'search_region_{i}'][0].bbox
...
def _resize_image_with_pad(name: str, context: dict,
                           output_size: Tuple[int, int],
                           interpolation_mode: str):
    image = context[f'{name}_image']
    output_size_hw = (output_size[1], output_size[0])  # (height, width)
    resized_image = resize_with_pad(image, output_size_hw, torchvision.transforms.InterpolationMode[interpolation_mode.upper()])
    new_height, new_width = _get_max_res_without_distortion(image.shape[-2:], output_size_hw)
    scaling_factor = (new_height / image.shape[-2], new_width / image.shape[-1])
    resized_bbox = bbox_scale(context[f'{name}_bbox'].astype(np.float64),
                              np.array((scaling_factor[1], scaling_factor[0]), dtype=np.float64))
    context[f'{name}_resized_image'] = resized_image
    context[f'{name}_resized_bbox'] = resized_bbox
...
class SPMTrackDataProcessorBatchCollator:
    ...
    def __call__(self, batch: Sequence[Mapping], collated: TrainData):
        ...
        for i in range(max_x + 1):
            data_dict[f'x_{i}'] = collate_element_as_torch_tensor(batch, f'x_{i}_resized_image').to(self.dtype)
