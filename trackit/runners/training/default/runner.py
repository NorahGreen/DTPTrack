...
from trackit.models import ModelInstance, ModelAuxiliaryBranchStateSavingMixin, _get_state_dict
...
class DefaultTrainer(Runner):
    ...
    def epoch_begin(self, epoch: int, task_name: Optional[str], is_train: bool,
        ...
        if self._ema is not None and self.is_train:
            self._ema.on_epoch_begin()

        if self.is_train:
            if self._ema is not None:
                self._model_updated_event_notifier = model_manager.create_external_updater(
                    lambda exclude_frozen_parameters: _get_state_dict(self._ema.get_model(), exclude_frozen_parameters))
            else:
                self._model_updated_event_notifier = self._model_instance
        self._epoch = epoch
    ...
    def epoch_end(self, epoch: int, *_):
        ...
        del self._epoch
        self._model = None
        self._model_updated_event_notifier = None
    ...
    def run(self, data: TrainData):
        ...
        with self._autograd_detect_anomaly_fn(), nullcontext() if update_grad or self._ddp_no_sync_fn is None else self._ddp_no_sync_fn():
            ...
            if self.is_train:
                self._iteration += 1
                if update_grad:
                    self._model_updated_event_notifier.notify_update()
