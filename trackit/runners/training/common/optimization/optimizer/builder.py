...
def build_optimizer(model: nn.Module, criterion: Optional[nn.Module],
    ...
    if optimizer_config['type'] == 'SGD':
    ...
    elif optimizer_config['type'] == 'stable_adamw':
        ...
        if max_grad_norm is not None and max_grad_norm > 0.:
            print('optimizing with StableAdamW does not require gradient clipping. Gradient clipping is disabled.')
    elif optimizer_config['type'] == 'muon':
        from .muon import Muon
        if 'momentum' in optimizer_config:
            optimizer_parameters['momentum'] = optimizer_config['momentum']
        if 'match_adamw_rms' in optimizer_config:
            optimizer_parameters['match_adamw_rms'] = optimizer_config['match_adamw_rms']
        if 'enable_torch_compile' in optimizer_config:
            optimizer_parameters['enable_torch_compile'] = optimizer_config['enable_torch_compile']
        optimizer_cls = Muon
    elif optimizer_config['type'] == 'muon_with_adam':
        ...
        if 'enable_torch_compile' in optimizer_config:
            optimizer_parameters['enable_torch_compile'] = optimizer_config['enable_torch_compile']
        if 'adam_lr' in optimizer_config:
            optimizer_parameters['adam_lr'] = optimizer_config['adam_lr']
        if 'adam_betas' in optimizer_config:
            optimizer_parameters['adam_betas'] = optimizer_config['adam_betas']
        if 'adam_eps' in optimizer_config:
            optimizer_parameters['adam_eps'] = optimizer_config['adam_eps']
