...
def apply_siamese_fusion_vit_lr_layer_decay_(rule: dict, base_lr: float, base_weight_decay: Optional[float],
                              module_parameters: Dict[str, nn.Parameter],
                              optimizer_param_dict: list,
                              decay_parameter_names: Tuple[str, ...]):
    fusion_blocks_param_path = rule['fusion_blocks']
    z_blocks_param_path = rule['z_blocks']
    x_blocks_param_path = rule['x_blocks']
    ...
    for module_parameter_name in list(module_parameters.keys()):
        if module_parameter_name.startswith(z_blocks_param_path):
            block_id = int(module_parameter_name[len(z_blocks_param_path) + 1:].split('.')[0])
            if block_id not in block_modules:
                block_modules[block_id] = {}
            block_modules[block_id][module_parameter_name] = module_parameters.pop(module_parameter_name)
        if module_parameter_name.startswith(x_blocks_param_path):
            block_id = int(module_parameter_name[len(x_blocks_param_path) + 1:].split('.')[0])
            if block_id not in block_modules:
                block_modules[block_id] = {}
            block_modules[block_id][module_parameter_name] = module_parameters.pop(module_parameter_name)

    feat_block_len = len(block_modules)

    for module_parameter_name in list(module_parameters.keys()):
        if module_parameter_name.startswith(fusion_blocks_param_path):
            block_id = int(module_parameter_name[len(fusion_blocks_param_path) + 1:].split('.')[0]) + feat_block_len
            if block_id not in block_modules:
                block_modules[block_id] = {}
            block_modules[block_id][module_parameter_name] = module_parameters.pop(module_parameter_name)
