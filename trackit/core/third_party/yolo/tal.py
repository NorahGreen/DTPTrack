...
class TaskAlignedAssigner(nn.Module):
    ...
    This class assigns ground-truth (Base) objects to anchors based on the task-aligned metric, which combines both
    ...
    def __init__(self, topk: int | None = 13, num_classes=80, alpha=1.0, beta=6.0, eps=1e-9):
    ...
    def get_box_metrics(self, pd_scores, pd_bboxes, gt_labels, gt_bboxes, mask_gt):
        ...
        # Get the scores of each grid for each Base cls
    ...
    def select_highest_overlaps(mask_pos, overlaps, n_max_boxes):
        ...
        if fg_mask.max() > 1:  # one anchor is assigned to multiple gt_bboxes
            ...
            fg_mask = mask_pos.sum(-2)
        # Find each grid serve which Base(index)
