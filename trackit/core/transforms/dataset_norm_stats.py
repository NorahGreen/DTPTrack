...
def get_dataset_norm_stats_transform(dataset: str, inplace: bool):
    ...
    if dataset == 'imagenet':
        ...
        return transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5], inplace=inplace)
    elif dataset == 'none':
...
def get_dataset_norm_stats_transform_reversed(dataset: str, inplace: bool):
    ...
    if dataset == 'imagenet':
        ...
        return transforms.Normalize(mean=[-m / s for m, s in zip([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])],
                                    std=[1 / s for s in [0.5, 0.5, 0.5]], inplace=inplace)
    elif dataset == 'none':
        return transforms.Lambda(lambda x: x if inplace else x.clone())
    else:
        raise NotImplementedError()
