...
def prepare_siamfc_cropping_with_augmentation_variant_allow_out_of_region(
        bbox: np.ndarray, area_factor: float,
        input_size: np.ndarray,
        output_size: np.ndarray,
        scale_jitter_factor: float, translation_jitter_factor: float,
        rng_engine: np.random.Generator = np.random.default_rng(),
        min_image_size: np.ndarray = np.array((1., 1.)),
        max_image_size: np.ndarray = np.array((np.inf, np.inf)),
        min_image_ratio: float = 0.0,
        max_image_ratio: float = 1.0,
        retry_count: int = 10):
    is_positive = True
    count = -1
    while True:
        count += 1
        if scale_jitter_factor == 0 and translation_jitter_factor == 0:
            scaling, translation = get_scale_and_translation_factors(bbox, area_factor, output_size)
            break

        scaling, translation = get_jittered_scale_and_translation_factors(bbox, area_factor, output_size,
                                                                          scale_jitter_factor,
                                                                          translation_jitter_factor,
                                                                          rng_engine)

        output_image_bbox = bbox_scale_and_translate(np.array((0, 0, input_size[0], input_size[1]), dtype=np.float64),
                                                     scaling, translation)

        if scale_jitter_factor == 0 and translation_jitter_factor == 0:
            break

        if not is_bbox_intersecting_image(output_image_bbox, output_size):
            if count > retry_count:
                is_positive = False
                break
            continue

        bbox_clip_to_image_boundary_(output_image_bbox, output_size)
        bbox_wh = bbox_get_width_and_height(output_image_bbox)

        if np.any(bbox_wh < min_image_size) or \
                np.any(bbox_wh > max_image_size) or \
                (np.prod(bbox_wh) > (output_size.prod() * max_image_ratio)) or \
                (np.prod(bbox_wh) < (output_size.prod() * min_image_ratio)):
            if count > retry_count:
                is_positive = False
                break
            continue
    ...
    cropping_parameter = np.stack((scaling, translation))
    is_object_intersecting_output_image = False
    if is_positive:
        output_object_bbox = bbox_scale_and_translate(bbox, scaling, translation)
        is_object_intersecting_output_image = is_bbox_intersecting_image(output_object_bbox, output_size)

    return cropping_parameter, is_positive, is_object_intersecting_output_image
